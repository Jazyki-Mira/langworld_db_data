...
def test___check_and_get_ids_from_hierarchy_fails_for_malformed_family_id():
    with pytest.raises(GenealogyValidatorError) as e:
        GenealogyValidator(
            file_with_hierarchy=DIR_WITH_VALIDATORS_TEST_FILES / 'genealogy_families_hierarchy_bad_malformed_id.yaml',
...
def test__check_ids_in_list_of_names_fails_for_file_with_malformed_family_id():
    validator = GenealogyValidator(
        file_with_names=DIR_WITH_VALIDATORS_TEST_FILES / 'genealogy_families_names_bad_malformed_id.csv',
...
def test__check_ids_in_list_of_names_fails_for_file_with_family_id_that_does_not_match_any_id_in_hierarchy():
    validator = GenealogyValidator(
        file_with_names=DIR_WITH_VALIDATORS_TEST_FILES / 'genealogy_families_names_bad_id_is_not_found_in_hierarchy.csv',
        file_with_hierarchy=GOOD_FILE_WITH_HIERARCHY
    )
