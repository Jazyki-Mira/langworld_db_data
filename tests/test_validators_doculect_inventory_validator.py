...
def test__init__fails_with_file_with_non_unique_doculect_ids():
    with pytest.raises(DoculectInventoryValidatorError, match='repeating values in column <id>: ukrainian'):
...
def test__check_family_ids_in_genealogy_fails_with_bad_file():
    with pytest.raises(DoculectInventoryValidatorError, match='genealogy family ID foobar not found'):
...
def test__match_doculects_to_files_fails_with_more_doculects_than_files():
    ...
    with pytest.raises(DoculectInventoryValidatorError, match='Doculect nogai has no file with feature profile'):
