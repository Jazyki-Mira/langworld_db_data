...
def check_yaml_file(path_to_file: Path, verbose: bool = True) -> None:
    ...
    top_level_dict_keys = [
        pattern_for_top_level_dict_keys.match(line).group('key') for line in data.splitlines()
    ...
    for key in counter:
        if counter[key] > 1:
            raise ParserError(f'File {path_to_file} contains more than one dictionary key <{key}> at the top level')
