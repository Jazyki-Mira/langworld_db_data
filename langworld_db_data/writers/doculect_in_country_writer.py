...
class DoculectInCountryWriter:
    ...
    def __init__(
        ...
        self.doculect_ids = [row[KEY_FOR_ID] for row in doculect_rows]
        ...
        self.doculect_id_to_countries: dict[str, set[str]] = {
            row[KEY_FOR_ID]: {row["main_country_id"]} for row in doculect_rows
        }
        ...
        for row in read_dicts_from_csv(file_with_countries):
            for locale in LOCALES:
                ...
                self.country_name_and_locale_to_country_id[(row[locale], locale)] = row[KEY_FOR_ID]
